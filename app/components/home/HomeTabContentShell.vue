...
<template>
  <div>
    <div class="flex items-center justify-between py-2 xl:hidden">
      ...
      >
        <Icon name="mage:filter-fill" size="24" aria-hidden="true" />
        <p class="text-medium">Filters</p>
      </button>
      <div
        v-if="showFilters && !isSubjectsTab"
        :class="[
        ...
        <div class="w-full h-full bg-white md:w-80">
          ...
          <div class="flex flex-col gap-4 mt-10">
            ...
            <button
              ...
              type="button"
              class="px-4 py-2 text-left cursor-pointer text-oceanBlue"
              @click="handleReset"
            >
              Reset filters
            </button>
    ...
    <div class="flex items-center justify-center w-full gap-4 xl:items-start">
      ...
      >
        <DropDownMenu
          ...
          :filter-value="[]"
          @emit-update-filter-value="handleFilterUpdate"
        />
        <button
          v-if="hasActiveFilter"
          type="button"
          class="px-2 pt-2 cursor-pointer text-oceanBlue"
          @click="handleReset"
        >
          Reset filters
        </button>
