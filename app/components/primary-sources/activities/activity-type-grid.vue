...
import { ACTIVITY_TYPES_CONFIG, type ActivityTypeConfig } from "@/shared/config/activity-types-config";
...
<template>
  <div class="space-y-6">
    ...
    <div v-if="props.allowSearch" class="flex flex-col gap-4 sm:flex-row">
      ...
      >
        <CardHeader class="pb-3">
          ...
          <div class="flex items-center justify-between gap-2">
            <CardTitle class="text-base text-oceanBlue">{{ activity.title }}</CardTitle>
            <Badge
              v-if="!activity.implemented"
              variant="outline"
    ...
    <div v-else class="py-12 text-center">
      ...
      <p class="text-sm text-slate-400">
        {{ searchTerm ? "Try adjusting your search terms" : "No activities match your current filters" }}
      </p>
      <Button
        v-if="searchTerm"
