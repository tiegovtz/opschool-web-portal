...
const { data, pending, error, refresh } = await useFetch<ActivitiesByTypeResponse>(
  "/api/primary/activities/one-per-type",
  {
    default: () => ({
      success: false,
      totalTypes: 0,
...
<template>
  <div class="min-h-screen bg-gradient-to-br from-sky-50 via-white to-amber-50">
      ...
      <section class="rounded-[2rem] border border-oceanBlue/10 bg-white/90 p-6 shadow-[0_24px_80px_-48px_rgba(1,61,96,0.55)] backdrop-blur md:p-8">
        <div class="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div class="max-w-3xl space-y-3">
            ...
            <h1 class="text-3xl font-black tracking-tight text-oceanBlue md:text-5xl">
        ...
        <div class="mt-6 grid gap-3 md:grid-cols-3">
          <div class="rounded-2xl bg-sky-50 p-4">
            <p class="text-xs font-semibold uppercase tracking-[0.22em] text-sky-700">Matched Types</p>
            <p class="mt-2 text-3xl font-black text-oceanBlue">{{ data?.matchedTypes ?? 0 }}</p>
          </div>
          <div class="rounded-2xl bg-amber-50 p-4">
            <p class="text-xs font-semibold uppercase tracking-[0.22em] text-amber-700">Missing Types</p>
            <p class="mt-2 text-3xl font-black text-oceanBlue">{{ data?.missingTypes?.length ?? 0 }}</p>
          </div>
          <div class="rounded-2xl bg-emerald-50 p-4">
            <p class="text-xs font-semibold uppercase tracking-[0.22em] text-emerald-700">Visible Cards</p>
            <p class="mt-2 text-3xl font-black text-oceanBlue">{{ filteredItems.length }}</p>
          </div>
        </div>
      </section>
      ...
      <section v-else class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        ...
        >
          ...
          <div class="mt-5 flex items-center justify-between border-t border-slate-100 pt-4">
            ...
            <span class="text-sm font-semibold text-oceanBlue transition group-hover:translate-x-1">
              Open
            </span>
          </div>
        </button>
    ...
    <Dialog :open="previewOpen" :on-open-change="handleDialogChange">
      <DialogContent class="max-h-[90vh] max-w-6xl overflow-hidden p-0">
        <DialogHeader class="border-b border-oceanBlue/10 bg-sky-50 px-6 py-5">
          ...
          <DialogDescription class="pr-10 text-sm text-slate-600">
            ...
            <span v-if="selectedActivityId">Activity ID: {{ selectedActivityId }}</span>
          </DialogDescription>
        </DialogHeader>
