...
<template>
  <div ref="dropdownRef" class="relative w-full text-left">
    ...
    <button
      type="button"
      class="flex items-center justify-between h-full w-full px-4 py-2 text-sm text-gray-700 rounded-md shadow-sm focus:outline-none"
      @click="isOpen = !isOpen"
    >
      <span :class="[
    ...
    <transition name="fade">
      <ul
        v-if="isOpen"
        class="absolute z-50 w-full mt-1 overflow-y-auto text-sm bg-white border border-gray-200 rounded-md shadow-lg max-h-32"
      >
        <li
