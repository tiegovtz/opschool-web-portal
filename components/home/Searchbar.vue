...
<template>
  <div
    :class="[
      ...
      appearance === 'normal'
        ? 'max-w-md'
       :`h-72 bg-background3 bg-cover bg-center bg-no-repeat rounded-md`,
    ]"
   ...
   <div
    ...
    <form
      ...
      <div class="flex items-center w-full">
        ...
        <input
            type="text"
            @input="inputSearch"
             v-model="searchReactive.search" placeholder="What do you want to learn?"
    ...
    <!-- Apperance Not Normal -->
     <form
        v-else
        action=""
        class="flex items-center w-full max-w-3xl bg-white rounded-md h-15 p-2"
      @submit.prevent="search"
      >
