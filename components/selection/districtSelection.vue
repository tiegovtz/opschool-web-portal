...
<template>
  <div class="flex flex-col items-start w-full" v-trusted>
    <label
      for="district"
      class="font-semibold capitalize text-oceanBlue text-extraSmall"
    ...
    v-trusted
      ...
      :list="
        data.district.map((district) => ({ id: district, name: district }))
      "
      :placeholder="'Select a District'"
      :modelValue="props.district"
      @updateModelValue="(value) => emit('updateDistrict', value)"
    />

    <p v-else-if="data.status === 'pending'" class="text-sm text-gray-400" v-trusted>Loading districts...</p>
    <p v-else-if="data.status === 'error'" class="text-sm text-red-500" v-trusted>
      {{ data.error }}
    </p>
    <p v-else class="text-sm text-gray-400" v-trusted>Select a region first.</p>
