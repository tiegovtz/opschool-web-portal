...
watch([props.region, props.district], ([region, district]) => {
  if (region && district) {
     fetchSchools(region, district);
  }
});
...
<template>
  <div class="flex w-full flex-col items-start">
    ...
    <select 
      name="school" 
      id="school" 
      class="w-full p-2 focus:outline-none focus:ring-0 capitalize" 
      :class="{ 'text-textGray/40': error }"
      @change="$emit('updateSchool', $event.target.value)"
    >
      ...
      <option value="" v-else-if="data.schools && data.status === 'success'">Eg (Taifa Secondary School) ...</option>
