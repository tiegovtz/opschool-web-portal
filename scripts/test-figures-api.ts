 ...
 * 
 * Usage:
 *   npx tsx scripts/test-figures-api.ts
 * 
 * Or with custom credentials:
 *   EMAIL=your@email.com PASSWORD=yourpassword npx tsx scripts/test-figures-api.ts
 */
 ...
 */
async function testFiguresEndpoint(token: string, options?: {
  limit?: number;
  offset?: number;
  subject?: string;
  category?: string;
...
async function main() {
  ...
  console.log(`Email: ${EMAIL || '(not provided - will prompt)'}`);
  console.log(`${'='.repeat(60)}\n`);
  ...
  if (!EMAIL || !PASSWORD) {
    ...
    console.error('\nUsage:');
    console.error('  EMAIL=your@email.com PASSWORD=yourpassword npx tsx scripts/test-figures-api.ts');
